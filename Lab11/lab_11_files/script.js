...
function validate() {
	var fileName = document.getElementById('tbFile').value;							//obtain the XML filename from the text box
	...
	if(!fileName) {											//check if a filename is entered
		...
		} else if(!fileName.match('nba.xml')) {  							//check if the filename is nba.xml
...
function init() {
	let submitElement = document.getElementById('btnExecute');
	submitElement.addEventListener("click", execute)
}
